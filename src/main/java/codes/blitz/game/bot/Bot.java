...
public class Bot
{
    ...
    ArrayList<Point> pufferSpot = new ArrayList<>();
    public Bot()
    {
    ...
    public Command getCommand(GameMessage gameMessage)
    {
        ...
        if (gameMessage.round().intValue() == 0)
        {
            ...
            LinkedHashMap<Integer, Point> pufferSpotMap = new LinkedHashMap<>();
            ...
            for (int i = 0; i < gameMap.height(); i++)
            {
                for (int j = 0; j < gameMap.width(); j++)
                {
                    if (mapD1[i][j] >= 2)
                    {
                        pufferSpotMap.put(mapD1[i][j], new Point(j, i));
                    }
                }
            }
            ...
            Map<Integer, Point> prout = new TreeMap<>(pufferSpotMap);
            for(Integer score: prout.keySet()) {

                System.out.println(pufferSpot + " = " + score);

                pufferSpot.add(pufferSpotMap.get(score));
            }
        }
        ...
        while(!pufferSpot.isEmpty() && fric >= 280) {
            Point newPoint = pufferSpot.get(0);
